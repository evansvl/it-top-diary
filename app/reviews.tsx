  ...
  return (
    <View className="mb-3 rounded-card bg-ink-800 p-4">
      <View className="flex-row items-start justify-between">
        <Text className="flex-1 pr-3 text-xs text-slate-400" numberOfLines={1}>
          {item.subject}
        </Text>
      ...
      <Text className="mt-2 text-sm leading-5 text-slate-100">
        {item.message}
      </Text>
      <Text className="mt-2 text-xs text-slate-500">{item.teacher}</Text>
    </View>
  );
}
